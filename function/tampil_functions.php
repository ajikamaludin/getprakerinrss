...
function tampil_post_by($email){
    $sql = "SELECT all_blog.judul_post,all_blog.tgl_post,pengguna.nama FROM `all_blog`,`pengguna` WHERE pengguna.email='$email' AND pengguna.id_pengguna=all_blog.id_pengguna ORDER BY `all_blog`.`tgl_post` DESC
 LIMIT 7 ";
    $result = result($sql);
    return $result;
}
